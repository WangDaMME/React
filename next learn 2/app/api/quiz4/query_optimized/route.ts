/**
 * 作用：API 路由，URL = /api/quiz4/query_optimized
 * - POST 接收 { q, limit }，用带缓存的索引方式查联系人，返回 { results, count }
 */
import { NextResponse } from 'next/server'
import type { ApiContactQueryResponse } from '@/types/contact'
import { validateQuery } from '@/lib/validation/query'
import { searchContactsOptimized } from '@/lib/quiz4/contactsStore'
...
export async function POST(request: Request) {
	...
	const { results } = await searchContactsOptimized(parsed.data.q, parsed.data.limit ?? 20)
